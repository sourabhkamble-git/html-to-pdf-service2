...
<head>
  ...
  <style>
    ...
    *[data-bullet="true"]::before,
    ...
    /* CRITICAL: Page break styles for PDF generation */
    @media print {
      /* Force page breaks for elements with page-break classes or styles */
      .page-break,
      .page-break-before,
      section.docx:not(:first-child),
      ...
      /* CRITICAL: Each section.docx after the first should start on a new page */
      ...
      /* Allow page breaks after table rows if needed */
      tr {
  ...
  <script>
    (async function() {
      try {
        ...
        // CRITICAL: Detect and preserve page breaks from Word document
        // docx-preview renders Word sections as <section class="docx"> elements
        // Each section often represents a page in Word, so we need to add page breaks between sections
        const sections = wrapper.querySelectorAll('section.docx');
        console.log('Found', sections.length, 'section(s) in docx-preview output');
        
        // Add page breaks before each section except the first one
        sections.forEach((section, index) => {
          if (index > 0) {
            // Add page break before this section (it's a new page in Word)
            section.style.pageBreakBefore = 'always';
            section.style.breakBefore = 'page';
            section.classList.add('page-break-before');
            console.log('Added page break before section', index + 1);
          }
        });
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (!existingStyle) {
        ...
        style.textContent = `
          @media print {
            /* CRITICAL: Force page breaks for elements with page-break classes or styles */
            .page-break,
            .page-break-before,
            [style*="page-break-before: always"],
            [style*="break-before: page"] {
